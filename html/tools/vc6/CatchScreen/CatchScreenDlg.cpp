...
{
 	if (pWnd == this &&m_rectTracker._xdSetCursor(this->m_hWnd, nHitTest)
 ...
 HBITMAP CCatchScreenDlg::CopyScreenToBitmap(LPRECT lpRect,BOOL bSave)
{
	...
	if(bSave)
	{
		...
		xdSave2File(hMemDC,hBitmap,"d:\\xdtest.bmp");
	} 
	DeleteDC(hScrDC);
	DeleteDC(hMemDC);
	return hBitmap;
 ...
 void CCatchScreenDlg::DrawTip()
{
	...
	{
		strTemp="\r\n press left button to select \r\n AOI \r\n\r\n press ESC or right button to quit.";
	}
	else
	if(m_bDraw&&m_bFirstDraw)
	{
		strTemp="\r\n release left button to finish select AOI\r\n\r\n press ESC to quit";
	}
	else
	if(m_bFirstDraw)
	{
		strTemp="\r\n use left button to select \r\n AOI \r\n\r\n Double click left button in AOI\r\n to save piture\r\n\r\n click right button to start again.";
	}
	string+=strTemp;
