...
void CMyTracker::Construct()
{
	...
	m_nStyle=CMyTracker::resizeMiddle|CMyTracker::solidLine;  
	m_rect.SetRect(-1,-2,-3,-4);
	//设置矩形颜色
	SetRectColor(RGB(10,100,130));
	//设置矩形调整时光标,默认的太小了,弄了个大点的
	SetResizeCursor(IDC_CURSOR6,IDC_CURSOR5,IDC_CURSOR2,IDC_CURSOR3,IDC_CURSOR4);
...
void CMyTracker::_xdDraw(CDC* pDC) const
{
	...
	if ((m_nStyle & (dottedLine|solidLine)) != 0)
	{
		...
		xdRect.InflateRect(+1, +1);   // borders are one pixel outside
		pDC->Rectangle(xdRect.left, xdRect.top, xdRect.right, xdRect.bottom);
		pDC->SetROP2(nOldROP);
	}
	...
	if ((m_nStyle & hatchInside) != 0)
	{
		...
		pDC->Rectangle(xdRect.left+1, xdRect.top+1, xdRect.right, xdRect.bottom);
		pDC->SetROP2(nOldROP);
	}
	...
	if ((m_nStyle & hatchedBorder) != 0)
	{
		...
		pDC->PatBlt(rectTrue.left, rectTrue.top, rectTrue.Width(),
			xdRect.top-rectTrue.top, 0x000F0001 /* Pn */);
		pDC->PatBlt(rectTrue.left, xdRect.bottom,
			rectTrue.Width(), rectTrue.bottom-xdRect.bottom, 0x000F0001 /* Pn */);
		pDC->PatBlt(rectTrue.left, xdRect.top, xdRect.left-rectTrue.left,
			xdRect.Height(), 0x000F0001 /* Pn */);
		pDC->PatBlt(xdRect.right, xdRect.top, rectTrue.right-xdRect.right,
			xdRect.Height(), 0x000F0001 /* Pn */);
	}
	...
	if ((m_nStyle & (resizeInside|resizeOutside)) != 0)
	{
		...
		for (int i = 0; i < 8; ++i)
		{
			if (mask & (1<<i))
			{
				GetHandleRect((TrackerHit)i, &xdRect);
				//改变当前调整手柄矩形颜色,也就是那八个点
				pDC->FillSolidRect(xdRect, m_rectColor);
			}
		}
	}
...
BOOL CMyTracker::_xdSetCursor(HWND hWnd, UINT nHitTest) const
{
